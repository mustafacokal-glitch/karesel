...
async function getSegmentator(onProgress?: (info: any) => void) {
  if (!segmentator) {
...
export async function removeBackground(imageData: ImageData, onProgress?: (info: any) => void): Promise<ImageData> {
  try {
    ...
    if (imageData.width > MAX_SIZE || imageData.height > MAX_SIZE) {
      ...
      if (!targetCtx) throw new Error('Target context is null');
    ...
    throw new Error('Arka plan temizlenirken bir hata oluştu: ' + message);
  }
}