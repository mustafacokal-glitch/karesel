...
const PALETTE_MAP: Record<number, any> = {};
PALETTE.forEach(p => {
  PALETTE_MAP[p.id] = p;
});
 ...
 */
function _getNeighbors(grid: number[][], r: number, c: number): number[] {
  return [
    grid[r - 1][c],       // P2 (üst)
 ...
 * @returns {number}
 */
function _countTransitions(neighbors: number[], foreground: number): number {
  let count = 0;
 ...
 */
export function safeBlockThinning(grid: number[][], outlineId: number): number[][] {
  try {
    if (!outlineId || outlineId <= 0) return grid;
    const rows = grid.length;
    const cols = grid[0].length;
 ...
 */
export function differentiateDuplicateColorNames(colorsMap: Record<number, any>): Record<number, any> {
  try {
    ...
    for (const [name, colors] of groups) {
      ...
      colors.sort((a: any, b: any) => a.luminance - b.luminance);
