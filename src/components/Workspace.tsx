...
export default function Workspace() {
  ...
  // Performance: Targeted Rendering (Dirty Rectangles) state
  const lastRenderedGridRef = useRef<(number | null)[][] | null>(null);
  ...
  const drawCanvas = useCallback(() => {
    ...
    const isResize = canvas.width !== svgW * dpr;
    if (isResize) {
    ...
    const currentConfigStr = `${showSolution}-${isEditMode}-${Object.keys(colorMap).length}`;
    const shouldFullRedraw = isResize || lastRenderConfigRef.current !== currentConfigStr || !lastRenderedGridRef.current;
    
    if (shouldFullRedraw) {
      // Temizle
      ctx.clearRect(0, 0, svgW, svgH);
      lastRenderConfigRef.current = currentConfigStr;
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        const lastCellId = lastRenderedGridRef.current?.[r]?.[c];

        // OPTIMIZATION: Targeted Rendering - Skip if cell hasn't changed
        if (!shouldFullRedraw && cellId === lastCellId) continue;
    ...
    // Save snapshot for next dirty render check
    lastRenderedGridRef.current = activeGrid.map(row => [...row]);
  }, [activeGrid, colorMap, isEditMode, showSolution, rows, cols, svgW, svgH]);

  // Hücreleri güncellemek için etkileşim koordinatlarını çöz
  const getGridCoordsFromEvent = useCallback((e: any) => {
    const canvas = canvasRef.current as HTMLCanvasElement | null;
  ...
  const handleStartInteraction = useCallback((e: any) => {
  ...
  const handleMoveInteraction = useCallback((e: any) => {
    ...
    <div className="bg-white rounded-2xl shadow-md p-5 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        <h3 className="text-sm font-semibold text-gray-700">
          🖼️ Çalışma Alanı
          {isEditMode && (
            <span className="ms-2 text-xs text-blue-500 font-medium">
              ✏️ Düzenleme modu
            </span>
          )}
        </h3>
